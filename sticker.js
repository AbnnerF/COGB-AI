...
async function buscarFigurinhasPorTema(termo, quantidade = 6) {
  ...
  function extrairUrls(valor, saida = new Set()) {
    ...
    if (typeof valor === 'string') {
      if (/^https?:\/\//i.test(valor) &&
          /\.(?:webp|png|jpg|jpeg)(?:\?|$)/i.test(valor)) {
        saida.add(valor);
      }
      // Algumas respostas retornam URL sem extensão.
      if (/^https?:\/\//i.test(valor) && /(?:sticker|image|cdn)/i.test(valor)) {
        saida.add(valor);
      }
    }
    return saida;
  ...
  try {
    ...
    if (!busca.ok) throw new Error(`smartSearch HTTP ${busca.status}`);
    const dadosBusca = await busca.json();
    const ids = [...extrairIds(dadosBusca)].slice(0, 10);

    // Fallback: algumas versões da API devolvem o id em links sticker.ly/s/ID.
    const links = new Set();
    JSON.stringify(dadosBusca).replace(/https?:\\?\/\\?\/sticker\.ly\/s\/([A-Za-z0-9_-]{4,40})/gi, (_, id) => {
      ids.push(id);
      return _;
    });
    ...
    // Também aceita diretamente URLs de imagens caso a busca já as devolva.
    extrairUrls(dadosBusca, urls);

    const lista = [...urls];
    // Embaralha para não devolver sempre as mesmas figurinhas.
    ...
    for (const url of lista) {
      ...
      try {
        ...
        // Garante que o WhatsApp receba WebP 512x512 mesmo quando a fonte for PNG/JPG/GIF.
        const webpBuffer = await converterParaWebp(buffer, 'recortada');
        const figurinha = await adicionarMetadados(webpBuffer, termo);
