...
async function iniciarBot() {
  ...
  sock.ev.on('messages.upsert', async ({ messages }) => {
    ...
    // Comandos de ação (/hug, /punch) - só funcionam em grupos
    // Usa APIs SFW com fallback para evitar que uma API única derrube os comandos.
    const ACOES_DISCORD = {
    ...
    async function obterUrlGifAcao(categoria) {
      ...
      // Gifukai tem endpoint SFW de hug e não exige chave.
      if (categoria === 'hug') {
        fontes.push({
          nome: 'Gifukai',
          url: 'https://api.gifukai.com/v1/hug',
          extrair: (dados) => dados?.url,
        });
      }

      // Waifu.pics possui endpoints SFW para hug e punch.
      fontes.push({
        nome: 'Waifu.pics',
      ...
      // Último fallback: Nekos.best, caso esteja acessível no Termux.
      fontes.push({
        nome: 'Nekos.best',
      ...
      for (const fonte of fontes) {
        try {
          ...
          if (!urlGif || typeof urlGif !== 'string') {
            throw new Error('a API não devolveu uma URL de GIF');
          }

          console.log(`GIF de ${categoria} obtido via ${fonte.nome}: ${urlGif}`);
          return urlGif;
        } catch (erro) {
          erros.push(`${fonte.nome}: ${erro.message}`);
          console.log(`Falha na fonte ${fonte.nome} para /${categoria}: ${erro.message}`);
        }
      }
    ...
    async function baixarGif(urlGif) {
      ...
      if (buffer.length < 1000) {
        throw new Error(`GIF baixado ficou pequeno demais (${buffer.length} bytes)`);
      }
