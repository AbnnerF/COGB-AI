...
async function iniciarBot() {
  ...
  sock.ev.on('messages.upsert', async ({ messages }) => {
    ...
    // Se a pessoa está criando um pack e mandou uma foto
    if ((msg.message.imageMessage || msg.message.videoMessage) && aguardandoPack[chaveEspera]?.fase === 'fotos') {
      ...
      try {
        ...
        if (ehVideoPack && !msg.message.videoMessage.gifPlayback) {
          ...
          if (duracao && duracao > DURACAO_MAXIMA_FIGURINHA) {
            await enviarMsg(sock, chatId, { text: `🎬 Esse vídeo tem ${duracao.toFixed(1)}s. Vou usar somente os primeiros ${DURACAO_MAXIMA_FIGURINHA}s na figurinha animada.` });
          }
        }
        ...
        if (dimensoes && ehDesproporcional(dimensoes)) {
          // Guarda esta mídia e pausa o recebimento até o usuário escolher o formato.
          sessaoPack.fase = 'escolhendo_formato';
          sessaoPack.midiaPendente = { buffer: bufferImagem, legenda, animada: ehVideoPack };
        ...
        // A foto vira WebP somente na montagem final. Não enviamos cada figurinha separadamente.
        const recebidasAntes = sessaoPack.figurinhas.length;
        sessaoPack.figurinhas.push({ buffer: bufferImagem, legenda, animada: ehVideoPack, modo: 'recortada' });
        ...
        if (recebidas >= sessaoPack.total) {
          ...
          await enviarMsg(sock, chatId, {
            text: `🛠️ *Montando seu pacote de figurinhas...*\n\n📦 Nome: *${sessaoPack.nomePack}*\n🖼️ Fotos: *${recebidas}/${sessaoPack.total}*\n\nAguarde, vou transformar as fotos e colocar tudo em UM único pacote. 📦✨`,
          });
          ...
          for (const item of sessaoPack.figurinhas) {
            try {
              ...
              console.log('Erro ao converter foto do pack:', e.message);
            }
          }

          if (!figurinhasProntas.length) {
            await enviarMsg(sock, chatId, { text: '❌ Não consegui transformar as fotos em figurinhas.' });
            return;
          }
          ...
          try {
            ...
            await enviarMsg(sock, chatId, {
              text: `🎉 *Pacote criado e enviado!*\n\n📦 *${sessaoPack.nomePack}*\n🖼️ ${figurinhasProntas.length} figurinha(s)\n\nEle foi enviado como um pacote único, igual ao formato de pacote de figurinhas do WhatsApp.`,
            });
          } catch (e) {
            console.log('Erro ao enviar stickerPackMessage:', e.message);
            await enviarMsg(sock, chatId, {
              text: '❌ Não consegui enviar o pacote nativo. Verifique se o Baileys do projeto foi atualizado para a versão incluída neste ZIP e rode *npm install* novamente.',
            });
          }
          return;
        }
        ...
        await enviarMsg(sock, chatId, {
          text: `✅ ${recebidas}/${sessaoPack.total} mídia(s) recebida(s)!${recebidasAntes === recebidas ? '' : ''}\n\nManda a próxima foto ou vídeo. 📸🎬\n\nPara cancelar: */cancel pack*`,
        });
        return;
      } catch (err) {
        console.log('Erro ao receber foto do pack:', err.message);
        sessaoPack.timeoutHandle = setTimeout(() => {
          delete aguardandoPack[chaveEspera];
        }, 10 * 60 * 1000);
        await enviarMsg(sock, chatId, { text: '❌ Deu erro ao processar essa foto. Tenta mandar outra.' });
        return;
      }
    }
    ...
    if (aguardandoPack[chaveEspera]?.fase === 'escolhendo_formato') {
      ...
      const modos = { '1': 'recortada', '2': 'original', '3': 'esticada', recortada: 'recortada', original: 'original', esticada: 'esticada' };
      ...
      sessaoPack.figurinhas.push({ buffer: pendente.buffer, legenda: pendente.legenda, animada: pendente.animada, modo });
      delete sessaoPack.midiaPendente;
      const recebidas = sessaoPack.figurinhas.length;
      if (recebidas >= sessaoPack.total) {
        delete aguardandoPack[chaveEspera];
        await enviarMsg(sock, chatId, { text: `🛠️ *Montando seu pacote de figurinhas...*\n\n📦 Nome: *${sessaoPack.nomePack}*\n🖼️ Mídias: *${recebidas}/${sessaoPack.total}*` });
        const figurinhasProntas = [];
        for (const item of sessaoPack.figurinhas) {
          try {
            const webpBuffer = item.animada ? await criarFigurinhaAnimada(item.buffer, item.legenda, item.modo) : await criarFigurinhaDeImagem(item.buffer, item.legenda, item.modo);
            if (webpBuffer) figurinhasProntas.push({ buffer: webpBuffer, emojis: ['🙂'] });
          } catch (e) { console.log('Erro ao converter mídia do pack:', e.message); }
        }
        if (!figurinhasProntas.length) { await enviarMsg(sock, chatId, { text: '❌ Não consegui transformar as mídias em figurinhas.' }); return; }
        try { await enviarPacoteDeFigurinhas(sock, chatId, figurinhasProntas, sessaoPack.nomePack, 'COGB-AI'); await enviarMsg(sock, chatId, { text: `🎉 *Pacote criado e enviado!*\n\n📦 *${sessaoPack.nomePack}*\n🖼️ ${figurinhasProntas.length} figurinha(s)` }); } catch (e) { console.log('Erro ao enviar stickerPackMessage:', e.message); await enviarMsg(sock, chatId, { text: '❌ Não consegui enviar o pacote nativo.' }); }
        return;
      }
      sessaoPack.fase = 'fotos';
      sessaoPack.timeoutHandle = setTimeout(() => { delete aguardandoPack[chaveEspera]; }, 10 * 60 * 1000);
      await enviarMsg(sock, chatId, { text: `✅ ${recebidas}/${sessaoPack.total} mídia(s) recebida(s)!\n\nManda a próxima foto ou vídeo. 📸🎬\n\nPara cancelar: */cancel pack*` });
      return;
    }
