...
const COMANDOS_CORRIGIVEIS = [
  ...
  'rpg', 'levels', 'inimigos', 'inimigo', 'historia', 'história', 'inventario', 'inventário', 'battle', 'party', 'create', 'cancel', 'perfil', 'hug', 'punch', 'kiss', 'kill', 'amongus', 'vote', 'skip', 'report', 'dado', 'moeda', 'caraoucoroa', 'call', 'music', 'imagem', 'img',
];
...
const amongUsJogos = {};
const amongUsTasks = [
  'Consertar a fiação', 'Baixar dados', 'Limpar filtro', 'Enviar amostra ao laboratório',
  'Calibrar o painel', 'Abastecer o motor', 'Organizar arquivos', 'Inspecionar o reator'
];
const amongUsLocais = ['Elétrica', 'Laboratório', 'Reator', 'Administração', 'Cafeteria', 'Navegação'];

function amongUsNovoJogo(chatId, criadorId) {
  return { chatId, criadorId, fase:'aguardando_party', jogadores:[], papeis:{}, tarefas:{}, concluidas:{}, vivos:{}, reuniao:false, votos:{}, corpos:[], rodada:1, filaTarefa:{} };
}
function amongUsEscolherTarefas() { return [...amongUsTasks].sort(()=>Math.random()-0.5).slice(0,3); }
function amongUsVivos(jogo) { return jogo.jogadores.filter(id => jogo.vivos[id]); }

async function amongUsEnviarTarefa(sock, id, jogo) {
  if (!jogo.vivos[id]) return;
  const tarefas=jogo.tarefas[id]||[];
  const feitas=jogo.concluidas[id]||0;
  if (jogo.papeis[id] === 'impostor') {
    await enviarMsg(sock,id,{text:'🔴 *AMONG US — PV SECRETO*\n\nVocê é o *IMPOSTOR*!\n\nFinja fazer tarefas e observe os outros. O bot avisará em segredo quando surgir uma oportunidade.'});
    return;
  }
  if (feitas >= tarefas.length) return;
  const local=amongUsLocais[Math.floor(Math.random()*amongUsLocais.length)];
  await enviarMsg(sock,id,{text:`🟢 *TAREFA ENCONTRADA*\n\n📍 Local: *${local}*\n🛠️ Tarefa: *${tarefas[feitas]}*\n\nQuer realizar a tarefa?\n*1* — Fazer\n*2* — Recusar por enquanto\n\n📊 Progresso: *${feitas}/${tarefas.length}*`});
}

async function amongUsIniciar(sock,jogo) {
  const n=jogo.jogadores.length;
  const qtdImp=n>=13?2:1;
  const ordem=[...jogo.jogadores].sort(()=>Math.random()-0.5);
  for(const id of jogo.jogadores){jogo.vivos[id]=true;jogo.concluidas[id]=0;jogo.tarefas[id]=amongUsEscolherTarefas();}
  for(let i=0;i<qtdImp;i++) jogo.papeis[ordem[i]]='impostor';
  for(const id of jogo.jogadores){
    if(!jogo.papeis[id]) jogo.papeis[id]='inocente';
    await enviarMsg(sock,id,{text:`🎭 *AMONG US — PAPEL SECRETO*\n\n${jogo.papeis[id]==='impostor'?'🔴 *Você é o IMPOSTOR!*':'🟢 *Você é um INOCENTE!*'}\n\nNão mostre esta mensagem para ninguém.`});
  }
  jogo.fase='jogando';
  await enviarMsg(sock,jogo.chatId,{text:`🚀 *AMONG US COMEÇOU!*\n\n👥 ${n} jogadores\n🔴 ${qtdImp} impostor${qtdImp>1?'es':''}\n📋 As tarefas serão enviadas individualmente no PV.`});
  for(const id of jogo.jogadores) await amongUsEnviarTarefa(sock,id,jogo);
}

async function amongUsChecarVitoria(sock,jogo){
  const vivos=amongUsVivos(jogo);
  const imp=vivos.filter(id=>jogo.papeis[id]==='impostor').length;
  const trip=vivos.length-imp;
  if(imp===0){await enviarMsg(sock,jogo.chatId,{text:'🎉 *TRIPULAÇÃO VENCEU!*\n\nTodos os impostores foram eliminados.'});delete amongUsJogos[jogo.chatId];return true;}
  if(imp>=trip){await enviarMsg(sock,jogo.chatId,{text:'🔴 *IMPOSTORES VENCERAM!*\n\nOs impostores alcançaram a maioria.'});delete amongUsJogos[jogo.chatId];return true;}
  const tripIds=vivos.filter(id=>jogo.papeis[id]==='inocente');
  const total=tripIds.reduce((a,id)=>a+(jogo.tarefas[id]?.length||0),0);
  const feitas=tripIds.reduce((a,id)=>a+(jogo.concluidas[id]||0),0);
  if(total>0 && feitas>=total){await enviarMsg(sock,jogo.chatId,{text:'🎉 *TRIPULAÇÃO VENCEU!*\n\nTodas as tarefas foram concluídas.'});delete amongUsJogos[jogo.chatId];return true;}
  return false;
}

async function amongUsResolverVotos(sock,jogo){
  const vivos=amongUsVivos(jogo);
  const cont={};
  for(const id of vivos){const v=jogo.votos[id];if(v&&v!=='skip')cont[v]=(cont[v]||0)+1;}
  const skips=vivos.filter(id=>jogo.votos[id]==='skip').length;
  const ranking=Object.entries(cont).sort((a,b)=>b[1]-a[1]);
  if(!ranking.length || skips>=ranking[0][1]){
    await enviarMsg(sock,jogo.chatId,{text:'⏭️ *VOTAÇÃO PULADA!*\n\nNinguém foi expulso.'});
  } else {
    const alvo=ranking[0][0];jogo.vivos[alvo]=false;
    await enviarMsg(sock,jogo.chatId,{text:`🚪 @${alvo.split('@')[0]} foi expulso da nave.\n🔎 Papel: *${jogo.papeis[alvo]==='impostor'?'Impostor':'Inocente'}*`,mentions:[alvo]});
  }
  jogo.votos={};jogo.reuniao=false;jogo.fase='jogando';jogo.rodada++;
  await amongUsChecarVitoria(sock,jogo);
}
// ---------------------------------------------------
...
async function iniciarBot() {
  ...
  sock.ev.on('messages.upsert', async ({ messages }) => {
    ...
    // Respostas privadas das tarefas do Among Us.
    if (!isGroup && !textoLower.startsWith('/') && amongUsJogos) {
      for (const [grupoId,jogo] of Object.entries(amongUsJogos)) {
        if(jogo.fase!=='jogando'||!jogo.jogadores.includes(remetenteId)||!jogo.vivos[remetenteId]) continue;
        if(jogo.papeis[remetenteId]!=='inocente') { await enviarMsg(sock,chatId,{text:'🔴 Você é o impostor. Finja realizar tarefas e aguarde uma oportunidade.'}); return; }
        const textoN=textoLower.trim(); const feitas=jogo.concluidas[remetenteId]||0; const tarefas=jogo.tarefas[remetenteId]||[];
        if(textoN==='1') {
          jogo.concluidas[remetenteId]=Math.min(feitas+1,tarefas.length);
          const n=jogo.concluidas[remetenteId];
          await enviarMsg(sock,chatId,{text:`🛠️ *Tarefa concluída!*\n\n📊 ${n}/${tarefas.length}`});
          await enviarMsg(sock,grupoId,{text:`✅ @${remetenteId.split('@')[0]} terminou uma tarefa!\n📊 Progresso individual: *${n}/${tarefas.length}*`,mentions:[remetenteId]});
          if(n<tarefas.length) await amongUsEnviarTarefa(sock,remetenteId,jogo); else await amongUsChecarVitoria(sock,jogo);
          return;
        }
        if(textoN==='2') { await enviarMsg(sock,chatId,{text:'⏸️ Você deixou a tarefa para depois. Responda *1* quando quiser fazê-la.'}); return; }
        if(textoN && textoN!=='1' && textoN!=='2') { await enviarMsg(sock,chatId,{text:'Responda *1* para fazer a tarefa ou *2* para recusar por enquanto.'}); return; }
      }
    }
...
🌟 Sua jornada Pokémon começou!` });
    ...
    if (isGroup && textoLower === '/amongus') {
      if (amongUsJogos[chatId]) { await enviarMsg(sock,chatId,{text:'🎮 Já existe uma partida neste grupo.'}); return; }
      amongUsJogos[chatId]=amongUsNovoJogo(chatId,remetenteId);
      await enviarMsg(sock,chatId,{text:'🚀 *AMONG US INICIADO!*\n\nUse */party amongUs @pessoa1 @pessoa2 ...* para escolher os participantes.\n\n👥 Mínimo: 4\n👥 Máximo: 20'});
      return;
    }
    if (isGroup && /^\/party\s+amongus\b/i.test(textoLimpo)) {
      const jogo=amongUsJogos[chatId];
      if(!jogo){await enviarMsg(sock,chatId,{text:'❌ Primeiro use */amongus*.'});return;}
      if(jogo.fase!=='aguardando_party'){await enviarMsg(sock,chatId,{text:'❌ A partida já começou.'});return;}
      const ids=idsUnicos([remetenteId,...(msg.message.extendedTextMessage?.contextInfo?.mentionedJid||[])]);
      if(ids.length<4){await enviarMsg(sock,chatId,{text:'⚠️ São necessários pelo menos *4 jogadores*.'});return;}
      if(ids.length>20){await enviarMsg(sock,chatId,{text:'⚠️ O limite é de *20 jogadores*.'});return;}
      jogo.jogadores=ids;jogo.fase='contagem';
      await enviarMsg(sock,chatId,{text:`👥 *PARTY DO AMONG US*\n\n${ids.map((id,i)=>`${i+1}. @${id.split('@')[0]}`).join('\n')}\n\n⏳ Preparem-se! A partida começa em 10 segundos.`,mentions:ids});
      setTimeout(()=>{if(amongUsJogos[chatId]===jogo&&jogo.fase==='contagem')amongUsIniciar(sock,jogo).catch(e=>console.log('Erro ao iniciar Among Us:',e.message));},10000);
      return;
    }
    if (isGroup && amongUsJogos[chatId] && textoLower === '/amongus cancel') { delete amongUsJogos[chatId]; await enviarMsg(sock,chatId,{text:'🛑 Among Us cancelado.'}); return; }
    if (isGroup && amongUsJogos[chatId] && textoLower === '/amongus status') { const j=amongUsJogos[chatId]; await enviarMsg(sock,chatId,{text:`🎮 *STATUS*\n\nFase: ${j.fase}\n👥 Vivos: ${amongUsVivos(j).length}/${j.jogadores.length}\n📋 Rodada: ${j.rodada}`}); return; }
    if (isGroup && amongUsJogos[chatId] && textoLower === '/amongus report') {
      const j=amongUsJogos[chatId]; if(j.fase!=='jogando'){await enviarMsg(sock,chatId,{text:'❌ Não dá para reportar agora.'});return;} j.fase='reuniao';j.reuniao=true;j.votos={};
      await enviarMsg(sock,chatId,{text:'🚨 *REUNIÃO!*\n\nConversem e votem com */vote @pessoa* ou */skip*.'});return;
    }
    if (isGroup && amongUsJogos[chatId] && textoLower === '/skip') {
      const j=amongUsJogos[chatId];if(!j.reuniao||!j.vivos[remetenteId])return; j.votos[remetenteId]='skip';
      if(Object.keys(j.votos).filter(id=>j.vivos[id]).length>=amongUsVivos(j).length)await amongUsResolverVotos(sock,j);else await enviarMsg(sock,chatId,{text:'🗳️ Seu voto foi registrado.'});return;
    }
    if (isGroup && amongUsJogos[chatId] && textoLower.startsWith('/vote ')) {
      const j=amongUsJogos[chatId];if(!j.reuniao||!j.vivos[remetenteId])return;const alvo=(msg.message.extendedTextMessage?.contextInfo?.mentionedJid||[])[0];
      if(!alvo||!j.vivos[alvo]){await enviarMsg(sock,chatId,{text:'👤 Mencione um jogador vivo.'});return;}j.votos[remetenteId]=alvo;
      if(Object.keys(j.votos).filter(id=>j.vivos[id]).length>=amongUsVivos(j).length)await amongUsResolverVotos(sock,j);else await enviarMsg(sock,chatId,{text:'🗳️ Seu voto foi registrado.'});return;
    }
    ...
    // /kill: eliminação fictícia, apenas como brincadeira.
    if (isGroup && (textoLower === '/kill' || textoLower.startsWith('/kill '))) {
      ...
      if(!alvo){await enviarMsg(sock,chatId,{text:'💀 Mencione alguém! Ex: */kill @pessoa*'});return;}
      await enviarMsg(sock,chatId,{text:`*@${alvo.split('@')[0]} foi eliminado(a) da brincadeira.* 💀`,mentions:[alvo]});return;
    }
    ...
    if (texto.trim().toLowerCase() === '/updates' || texto.trim().toLowerCase() === '/update') {
      await enviarMsg(sock,chatId,{text:`🚀 *CHAIM-BOT — ATUALIZAÇÕES*\n\n🎮 *Brincadeiras*\n• Among Us com papéis secretos, tarefas no PV, reuniões, votos e skip.\n• /kill e /kiss adicionados às ações.\n• /dado e /moeda disponíveis.\n• /tops reúne vitórias das brincadeiras.\n\n🐾 *Pokémon Beta 0.1*\n• /pokemon, /choose, /pokemons, /mapa, /viajar, /capturar, /shop e /buy.\n• Grandes lendários e trios continuam reservados para futuras versões.\n\n🎵 *Música*\n• /play com busca aproximada e links do YouTube/Spotify.\n\n🤖 *IA*\n• /bot pode acompanhar a conversa do grupo e participar espontaneamente.\n\n🎨 *Figurinhas*\n• /create pack aceita fotos e vídeos em sequência.\n• Conversão de WebP animado está sendo tratada separadamente.\n\n🛠️ *Correções*\n• /menu agora reúne as funções disponíveis.\n• Correção automática de pequenos erros nos comandos.`});
      return;
    }
...
• /punch @pessoa — GIF
• /kiss @pessoa — GIF
• /kill @pessoa — Eliminação fictícia

🎵 *MÚSICA*
• /play <música> — Busca aproximada
• /audio — Extrair áudio
• /music call — Música em call (experimental)
...
• /updates
• /feedback <texto>
• /call — Chamada experimental
• /ligado

💡 O Chaim-Bot também tenta corrigir pequenos erros de digitação nos comandos.`});
      return;
    }
