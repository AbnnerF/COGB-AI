...
async function iniciarBot() {
  ...
  sock.ev.on('messages.upsert', async ({ messages }) => {
    ...
    // Comando /pack <tema> - procura packs públicos e envia 6 figurinhas aleatórias
    if (textoLower.startsWith('/pack ')) {
      ...
      const pack = await buscarFigurinhasPorTema(temaPack, 6);
      if (!pack.length) {
        await enviarMsg(sock, chatId, {
      ...
      await enviarMsg(sock, chatId, {
        text: `✅ *Encontrei ${pack.length} figurinhas!*\n\nTema: *${temaPack}*\n🎲 Foram escolhidas aleatoriamente dos packs encontrados.`,
      });
