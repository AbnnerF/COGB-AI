...
🌟 Sua jornada Pokémon começou!` });
    ...
    if (isGroup && ACOES_DISCORD[primeiraPalavra]) {
      ...
      try {
        const fontes = [];
        if (acao.categoria === 'hug') fontes.push({ nome: 'Gifukai', url: 'https://api.gifukai.com/v1/hug', extrair: d => d?.url });
        if (acao.categoria === 'kiss') fontes.push({ nome: 'Gifukai', url: 'https://api.gifukai.com/v1/kiss', extrair: d => d?.url });
        fontes.push({ nome: 'Waifu.pics', url: `https://api.waifu.pics/sfw/${acao.categoria}`, extrair: d => d?.url });
        fontes.push({ nome: 'Nekos.best', url: `https://nekos.best/api/v2/${acao.categoria}`, extrair: d => d?.results?.[0]?.url });

        let urlGif = null;
        const errosGif = [];
        for (const fonte of fontes) {
          try {
            const respostaApi = await fetch(fonte.url, {
            ...
            urlGif = fonte.extrair(dados);
            if (!urlGif) throw new Error('sem URL de GIF');
            break;
          } catch (e) {
            errosGif.push(`${fonte.nome}: ${e.message}`);
          }
        }
        if (!urlGif) throw new Error(`nenhuma API de GIF respondeu (${errosGif.join(' | ')})`);

        let bufferGif;
        try {
          const respostaGif = await fetch(urlGif, {
            headers: { 'User-Agent': 'ChaimBot/1.0' },
    ...
    // /menu — usa a mensagem personalizada centralizada em mensagens.js.
