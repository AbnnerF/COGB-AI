  ...
  sock.ev.on('messages.upsert', async ({ messages }) => {
    ...
    if (msg.message.imageMessage && aguardandoPack[chaveEspera]?.fase === 'fotos') {
      ...
      try {
        ...
        sessaoPack.figurinhas.push({ bufferImagem, legenda });
        ...
        if (recebidas >= sessaoPack.total) {
          ...
          for (const item of sessaoPack.figurinhas) {
            try {
              const webpBuffer = await criarFigurinhaDeImagem(item.bufferImagem, item.legenda, 'recortada');
              if (webpBuffer) {
        ...
        await enviarMsg(sock, chatId, {
          text: `✅ ${recebidas}/${sessaoPack.total} foto(s) recebida(s)!${recebidasAntes === recebidas ? '' : ''}\n\nManda a próxima foto. 📸\n\nPara cancelar: */cancel pack*`,
        });
        return;
      } catch (err) {
