...
async function iniciarBot() {
  ...
  sock.ev.on('messages.upsert', async ({ messages }) => {
    ...
    // Comando /pack <tema> - gera um pack de 6 figurinhas originais com IA
    if (textoLower.startsWith('/pack ')) {
      const temaPack = textoLimpo.slice(6).trim();
      if (!temaPack) {
        await enviarMsg(sock, chatId, {
          text: '🎨 Use assim: /pack memes\nExemplo: /pack gatos engraçados',
        });
        return;
      }
      ...
      await enviarMsg(sock, chatId, {
        text: `🎨 *Criando pack: ${temaPack}*\n\nVou gerar 6 figurinhas originais. Pode levar um pouquinho ⏳`,
      });

      const pack = await gerarPackDeFigurinhas(temaPack, 6);
      if (!pack.length) {
        await enviarMsg(sock, chatId, { text: '❌ Não consegui gerar o pack agora. Tenta outro tema.' });
        return;
      }

      await enviarMsg(sock, chatId, {
        text: `✅ *Pack pronto!*\n\nTema: *${temaPack}*\nFigurinhas geradas: *${pack.length}/6* 🎉`,
      });
