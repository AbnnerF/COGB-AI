...
const COMANDOS_CORRIGIVEIS = [
  'menu', 'updates', 'update', 'bot', 'help', 'p', 'pokemon', 'choose', 'mapa', 'viajar', 'capturar', 'pokemons', 'play', 'audio', 'fig', 'forca', 'duelo', 'tops', 'top', 'dado', 'moeda', 'caraoucoroa',
  'pack', 'packs', 'convert', 'quiz', 'party', 'feedback', 'ligado',
  'rpg', 'levels', 'inimigos', 'inimigo', 'historia', 'história', 'inventario', 'inventário', 'battle', 'party', 'create', 'cancel', 'perfil', 'hug', 'punch', 'kiss', 'imagem', 'img',
];
...
// -------------------- AMONG US (BETA) --------------------
const amongUsAtivos = {};
const AMONGUS_LOCAIS = ['Cafeteria','Elétrica','Administração','MedBay','Segurança','Armazém','Navegação','Reator','Comunicações'];
const AMONGUS_TASKS = ['passar o cartão','organizar os cabos','baixar dados','calibrar o reator','limpar o filtro','conectar fios','inspecionar a amostra','alinhar a nave'];
function amongRandom(arr) { return arr[Math.floor(Math.random() * arr.length)]; }
function amongPickImpostors(ids) {
  const pool = [...ids], total = ids.length >= 13 ? 2 : 1, result = [];
  while (result.length < total && pool.length) result.push(pool.splice(Math.floor(Math.random()*pool.length),1)[0]);
  return result;
}
async function amongFinish(sock, jogo, vencedor) {
  if (!jogo || jogo.fase === 'finalizado') return;
  jogo.fase = 'finalizado';
  const imp = jogo.impostores || [];
  const mentions = [...imp];
  const text = vencedor === 'impostores'
    ? `🔴 *OS IMPOSTORES VENCERAM!*\n\nOs impostores conseguiram dominar a partida.\n\n🕵️ Impostores: ${imp.map(id => `@${id.split('@')[0]}`).join(', ')}`
    : `🟢 *OS TRIPULANTES VENCERAM!*\n\nA tripulação conseguiu completar as tarefas ou eliminar os impostores.\n\n🕵️ Impostores: ${imp.map(id => `@${id.split('@')[0]}`).join(', ')}`;
  await enviarMsg(sock, jogo.grupoId, { text, mentions });
  delete amongUsAtivos[jogo.grupoId];
}
async function amongCheckWin(sock, jogo) {
  if (!jogo || jogo.fase === 'finalizado') return true;
  if ((jogo.tasksFeitas || 0) >= jogo.tasksTotais) { await amongFinish(sock, jogo, 'tripulantes'); return true; }
  const vivosImp = jogo.impostores.filter(id => jogo.vivos.includes(id));
  if (!vivosImp.length) { await amongFinish(sock, jogo, 'tripulantes'); return true; }
  const vivosTrip = jogo.vivos.length - vivosImp.length;
  if (vivosImp.length >= vivosTrip) { await amongFinish(sock, jogo, 'impostores'); return true; }
  return false;
}
async function amongRound(sock, grupoId) {
  const jogo = amongUsAtivos[grupoId];
  if (!jogo || jogo.fase !== 'jogando') return;
  if (await amongCheckWin(sock, jogo)) return;
  jogo.turno++;
  jogo.local = amongRandom(AMONGUS_LOCAIS);
  const jogador = amongRandom(jogo.vivos);
  const task = amongRandom(AMONGUS_TASKS);
  jogo.tarefaAtual = { jogador, task, local: jogo.local };
  await enviarMsg(sock, grupoId, { text: `📢 *MOVIMENTO DA NAVE*\n\n📍 Local: *${jogo.local}*\n👤 @${jogador.split('@')[0]} encontrou uma tarefa.\n\n🤖 A tarefa foi enviada no PV de @${jogador.split('@')[0]}.\n📋 Tarefas: *${jogo.tasksFeitas}/${jogo.tasksTotais}*`, mentions:[jogador] });
  await enviarMsg(sock, jogador, `🔧 Você encontrou uma tarefa em *${jogo.local}*: *${task}*.\n\nQuer fazer?\n\n1️⃣ Sim\n2️⃣ Não`);

  if (jogo.vivos.length >= 2 && Math.random() < 0.7) {
    const a = amongRandom(jogo.vivos);
    let b = amongRandom(jogo.vivos);
    while (b === a) b = amongRandom(jogo.vivos);
    const imp = jogo.impostores.includes(a) ? a : jogo.impostores.includes(b) ? b : null;
    const alvo = imp === a ? b : imp === b ? a : null;
    jogo.encontro = { a, b, imp, alvo, local:jogo.local };
    await enviarMsg(sock, grupoId, { text:`👀 *ENCONTRO*\n\n@${a.split('@')[0]} e @${b.split('@')[0]} estão juntos em *${jogo.local}*.`, mentions:[a,b] });
    if (imp && alvo) {
      jogo.oportunidade = { impostor:imp, alvo, local:jogo.local };
      await enviarMsg(sock, imp, `🤫 Você está sozinho com @${alvo.split('@')[0]} em *${jogo.local}*.\n\n1️⃣ Eliminar\n2️⃣ Deixar para lá`, { mentions:[alvo] });
    }
  }
  setTimeout(() => amongRound(sock, grupoId), 15000);
}
async function amongStart(sock, jogo) {
  jogo.impostores = amongPickImpostors(jogo.participantes);
  jogo.vivos = [...jogo.participantes]; jogo.tasks = {}; jogo.tasksFeitas=0; jogo.tasksTotais=jogo.participantes.length*3;
  jogo.fase='jogando'; jogo.turno=0; jogo.corpos=[]; jogo.votos={}; jogo.oportunidade=null;
  for (const id of jogo.participantes) {
    jogo.tasks[id]={feitas:0};
    const role=jogo.impostores.includes(id);
    try { await enviarMsg(sock,id,{text:role?'🔴 *AMONG US — SEU PAPEL*\n\nVocê é o *IMPOSTOR*. Seu papel é secreto.':'🟢 *AMONG US — SEU PAPEL*\n\nVocê é um *TRIPULANTE*. Complete tarefas e descubra os impostores.'}); } catch(e) {}
  }
  await enviarMsg(sock,jogo.grupoId,{text:`🚀 *AMONG US — PARTIDA INICIADA!*\n\n📍 A nave está em movimento.\n👥 ${jogo.participantes.length} jogadores\n📋 Tarefas: *0/${jogo.tasksTotais}*\n\n🔒 Cada jogador recebeu seu papel no PV.\n\nComandos: */amongus status*, */amongus report*, */vote @pessoa* ou */skip*.`});
  setTimeout(()=>amongRound(sock,jogo.grupoId),4000);
}
async function amongVoteEnd(sock, grupoId) {
  const jogo=amongUsAtivos[grupoId]; if(!jogo || jogo.fase!=='reuniao') return;
  const counts={}; for(const v of Object.values(jogo.votos||{})) counts[v]=(counts[v]||0)+1;
  const entries=Object.entries(counts).sort((a,b)=>b[1]-a[1]);
  if(!entries.length) { jogo.fase='jogando'; return; }
  const top=entries[0][1], leaders=entries.filter(([,n])=>n===top).map(([id])=>id);
  if(leaders.length!==1 || leaders[0]==='skip') { jogo.fase='jogando'; jogo.corpos=[]; await enviarMsg(sock,grupoId,{text:'🗳️ *VOTAÇÃO ENCERRADA*\n\n🤝 Empate ou skip. Ninguém foi expulso.\n\n🚀 A partida continua!'}); return setTimeout(()=>amongRound(sock,grupoId),5000); }
  const out=leaders[0]; jogo.vivos=jogo.vivos.filter(id=>id!==out); const imp=jogo.impostores.includes(out);
  await enviarMsg(sock,grupoId,{text:`🚪 @${out.split('@')[0]} foi expulso da nave.\n\n🔎 Era ${imp?'*IMPOSTOR*':'*TRIPULANTE*'}.`,mentions:[out]});
  jogo.corpos=[]; jogo.fase='jogando'; if(await amongCheckWin(sock,jogo)) return; setTimeout(()=>amongRound(sock,grupoId),5000);
}
...
🌟 Sua jornada Pokémon começou!` });
    ...
    if (isGroup && textoLower === '/amongus') {
      if(amongUsAtivos[chatId]){await enviarMsg(sock,chatId,{text:'⚠️ Já existe uma partida de Among Us neste grupo.'});return;}
      amongUsAtivos[chatId]={grupoId:chatId,criadorId:remetenteId,participantes:[],fase:'aguardandoParty'};
      await enviarMsg(sock,chatId,{text:`🚀 *AMONG US — LOBBY*\n\nUse:\n*/party amongUs @pessoa1 @pessoa2 ...*\n\n📌 Mínimo: 4 jogadores\n📌 4–12 jogadores: 1 impostor\n📌 13–20 jogadores: 2 impostores\n\n🔒 Os papéis serão enviados em privado.`}); return;
    }


    // /party amongUs — lobby do Among Us
    if (isGroup && textoLower.startsWith('/party amongus')) {
      const jogo=amongUsAtivos[chatId];
      if(!jogo || jogo.fase!=='aguardandoParty'){ await enviarMsg(sock,chatId,{text:'❌ Primeiro use */amongus*.'}); return; }
      if(remetenteId!==jogo.criadorId){ await enviarMsg(sock,chatId,{text:'❌ Só quem criou o lobby pode escolher os participantes.'}); return; }
      const ids=idsUnicos(msg.message.extendedTextMessage?.contextInfo?.mentionedJid||[]);
      jogo.participantes=idsUnicos([remetenteId,...ids]);
      if(jogo.participantes.length<4){await enviarMsg(sock,chatId,{text:'⚠️ O Among Us precisa de pelo menos *4 jogadores*.'});return;}
      if(jogo.participantes.length>20){await enviarMsg(sock,chatId,{text:'⚠️ O limite do beta é *20 jogadores*.'});return;}
      await enviarMsg(sock,chatId,{text:`👥 *LOBBY CONFIRMADO!*\n\n${jogo.participantes.map((id,i)=>`${i+1}. @${id.split('@')[0]}`).join('\n')}\n\n⏳ Começando em *10 segundos*...\n🔒 Os papéis serão enviados no PV.`,mentions:jogo.participantes});
      jogo.fase='contagem'; let n=10; const timer=setInterval(async()=>{n--;if(!amongUsAtivos[chatId]||jogo.fase!=='contagem'){clearInterval(timer);return;}if(n<=0){clearInterval(timer);await amongStart(sock,jogo);}},1000); return;
    }
    ...
    if (isGroup && textoLower === '/amongus status') { const j=amongUsAtivos[chatId]; if(!j){await enviarMsg(sock,chatId,{text:'ℹ️ Não há partida de Among Us ativa.'});return;} await enviarMsg(sock,chatId,`🚀 *STATUS*\n\n👥 Vivos: *${j.vivos?.length||j.participantes.length}/${j.participantes.length}*\n📋 Tarefas: *${j.tasksFeitas||0}/${j.tasksTotais||0}*\n📍 Local: *${j.local||'Lobby'}*\n🔄 Rodada: *${j.turno||0}*`); return; }
    if (isGroup && textoLower === '/amongus report') { const j=amongUsAtivos[chatId]; if(!j||j.fase!=='jogando'){await enviarMsg(sock,chatId,{text:'❌ Não há partida ativa para reportar.'});return;} if(!j.corpos?.length){await enviarMsg(sock,chatId,{text:'ℹ️ Não há corpo para reportar agora.'});return;} j.fase='reuniao';j.votos={};await enviarMsg(sock,chatId,{text:`🚨 *REUNIÃO DE EMERGÊNCIA!*\n\n💬 Conversem e tentem descobrir quem é suspeito.\n\n*/vote @pessoa* para votar ou */skip* para pular.\n⏱️ A votação dura até 60 segundos.`});setTimeout(()=>amongVoteEnd(sock,chatId),60000);return; }
    if (isGroup && textoLower.startsWith('/vote ')) { const j=amongUsAtivos[chatId]; if(!j||j.fase!=='reuniao'||!j.vivos.includes(remetenteId))return;const alvo=msg.message.extendedTextMessage?.contextInfo?.mentionedJid?.[0];if(!alvo||!j.vivos.includes(alvo)){await enviarMsg(sock,chatId,{text:'Mencione um jogador vivo.'});return;}j.votos[remetenteId]=alvo;await enviarMsg(sock,chatId,{text:`🗳️ @${remetenteId.split('@')[0]} votou.`,mentions:[remetenteId]});if(Object.keys(j.votos).length>=j.vivos.length)await amongVoteEnd(sock,chatId);return; }
    if (isGroup && textoLower === '/skip') { const j=amongUsAtivos[chatId]; if(!j||j.fase!=='reuniao'||!j.vivos.includes(remetenteId))return;j.votos[remetenteId]='skip';await enviarMsg(sock,chatId,{text:`⏭️ @${remetenteId.split('@')[0]} votou em skip.`,mentions:[remetenteId]});if(Object.keys(j.votos).length>=j.vivos.length)await amongVoteEnd(sock,chatId);return; }
    if (isGroup && textoLower === '/amongus cancel') { const j=amongUsAtivos[chatId]; if(!j){await enviarMsg(sock,chatId,{text:'ℹ️ Não há partida ativa.'});return;}if(remetenteId!==j.criadorId){await enviarMsg(sock,chatId,{text:'❌ Só o criador pode cancelar.'});return;}delete amongUsAtivos[chatId];await enviarMsg(sock,chatId,{text:'🛑 Among Us cancelado.'});return; }

    // Respostas secretas no PV: tarefas e decisão do impostor.
    if (!isGroup) { for (const j of Object.values(amongUsAtivos)) { if(j.fase!=='jogando'||!j.vivos.includes(remetenteId))continue; const r=textoLower.trim();
      if(j.tarefaAtual?.jogador===remetenteId && ['1','sim','s'].includes(r)){j.tasks[remetenteId].feitas++;j.tasksFeitas++;await enviarMsg(sock,remetenteId,{text:`✅ Tarefa concluída!\n\n📋 Você: ${j.tasks[remetenteId].feitas}/3`});await enviarMsg(sock,j.grupoId,{text:`✅ @${remetenteId.split('@')[0]} terminou uma tarefa!\n📋 Progresso: *${j.tasksFeitas}/${j.tasksTotais}*`,mentions:[remetenteId]});await amongCheckWin(sock,j);return;}
      if(j.tarefaAtual?.jogador===remetenteId && ['2','nao','não','n'].includes(r)){await enviarMsg(sock,remetenteId,{text:'↩️ Você deixou a tarefa para depois.'});return;}
      if(j.oportunidade?.impostor===remetenteId && ['1','2'].includes(r)){const op=j.oportunidade;if(r==='1'&&j.vivos.includes(op.alvo)){j.vivos=j.vivos.filter(id=>id!==op.alvo);j.corpos.push({jogador:op.alvo,local:op.local});await enviarMsg(sock,j.grupoId,{text:`🚨 *CORPO ENCONTRADO!*\n\nUm jogador foi encontrado em *${op.local}*.\n\nUse */amongus report* para iniciar uma reunião.`});}else{await enviarMsg(sock,remetenteId,{text:'🟢 Você decidiu deixar para lá.'});}j.oportunidade=null;await amongCheckWin(sock,j);return;}
    }}

    // ==================== BRINCADEIRAS RÁPIDAS ====================
    if (isGroup && textoLower === '/dado') {
      const n = Math.floor(Math.random() * 6) + 1;
      await enviarMsg(sock, chatId, { text: `🎲 @${remetenteId.split('@')[0]} rolou o dado e tirou *${n}*!`, mentions:[remetenteId] });
      return;
    }
    if (isGroup && (textoLower === '/moeda' || textoLower === '/caraoucoroa')) {
      const resultado = Math.random() < 0.5 ? 'CARA' : 'COROA';
      await enviarMsg(sock, chatId, { text: `🪙 @${remetenteId.split('@')[0]} jogou a moeda...\n\n➡️ *${resultado}!*`, mentions:[remetenteId] });
      return;
    }
    ...
    if (texto.trim().toLowerCase() === '/updates' || texto.trim().toLowerCase() === '/update') {
      await enviarMsg(sock, chatId, {
        text: `🚀 *ATUALIZAÇÕES DO COGB-AI*\n\n` +
          ...
          `• /hug @pessoa — abraço por GIF\n` +
          `• /punch @pessoa — ação por GIF\n` +
          `• /kiss @pessoa — ação por GIF\n\n` +
          `🎨 *Figurinhas*\n` +
    ...
    if (texto.trim().toLowerCase() === '/menu') {
      await enviarMsg(sock, chatId, {
        text: `╭━━━━━━━━━━━━━━━━━━━━━━━━━━╮\n` +
          ...
          `┃ └ Veja os rankings\n` +
          `┃ 🤗 */hug @pessoa*\n` +
          `┃ └ Envie um GIF de abraço\n` +
          `┃ 💋 */kiss @pessoa*\n` +
          `┃ └ Envie um GIF de beijo\n` +
          `╰━━━━━━━━━━━━━━━━━━━━━━━━━━╯\n\n` +
          `╭━━〔 🎵 *MÚSICA* 〕━━╮\n` +
          `┃ 🎵 */play <música>*\n` +
          ...
          `╭━━〔 🤖 *IA & UTILIDADES* 〕━━╮\n` +
          `┃ 🤖 */bot*\n` +
          `┃ └ Ativa o Chaim-Bot como participante do grupo\n` +
          `┃ 🧠 */ia*\n` +
