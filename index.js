...
const COMANDOS_CORRIGIVEIS = [
  ...
  'rpg', 'pokemon', 'pokémon', 'choose', 'chose', 'viajar', 'mapa', 'pokemons', 'capturar', 'shop', 'buy', 'pvp', 'trade', 'h', 'levels', 'inimigos', 'inimigo', 'historia', 'história', 'inventario', 'inventário', 'battle', 'party', 'create', 'cancel', 'perfil', 'hug', 'punch', 'imagem', 'img',
];
...
async function iniciarBot() {
  ...
  sock.ev.on('messages.upsert', async ({ messages }) => {
    ...
    // -------------------- POKÉMON BETA 0.1 --------------------
    if (comando === '/pokemon' || comando === '/pokémon') {
      await enviarMsg(sock, chatId, { text: pokemon.textoPerfil(remetenteId, nomeRemetente) });
      return;
    }

    if (comando === '/choose') {
      const escolha = textoLimpo.split(/\s+/).slice(1).join(' ').trim();
      if (!escolha) {
        await enviarMsg(sock, chatId, { text: pokemon.menuInicial() });
        return;
      }
      const resultadoEscolha = pokemon.escolher(remetenteId, escolha);
      if (!resultadoEscolha.ok) {
        if (resultadoEscolha.motivo === 'ja_escolheu') {
          const atual = resultadoEscolha.player.pokemons[0];
          await enviarMsg(sock, chatId, { text: `❌ Você já escolheu seu Pokémon inicial!\n\n${atual.emoji} *${atual.nome}* é o seu Pokémon inicial e essa escolha não pode ser feita novamente.` });
        } else {
          await enviarMsg(sock, chatId, { text: `❌ Escolha inválida.\n\nUse uma destas opções:\n1️⃣ 🌱 Bulbasaur\n2️⃣ 🔥 Charmander\n3️⃣ 💧 Squirtle\n\nExemplo: */choose 1*` });
        }
        return;
      }
      const s = resultadoEscolha.starter;
      await enviarMsg(sock, chatId, { text: `🎉 *SUA JORNADA COMEÇOU!*\n\n${s.emoji} *${s.nome}* foi escolhido como seu Pokémon inicial!\n🔹 Tipo: ${s.tipo}\n⭐ Nível: 5\n📍 Você começa na *Cidade de Pallet*.\n\n⚠️ Essa escolha ficou registrada permanentemente na sua conta.\n\nUse */viajar* quando quiser começar a explorar o mundo Pokémon!` });
      return;
    }

    if (comando === '/help' && /^\/help\s+(p|pokemon|pokémon)$/i.test(textoLimpo)) {
      await enviarMsg(sock, chatId, { text: pokemon.textoHelp() });
      return;
    }
    ...
    if (comando === '/mapa') {
      const p = pokemon.perfil(remetenteId);
      if (!p) {
        await enviarMsg(sock, chatId, { text: pokemon.menuInicial() });
        return;
      }
      await enviarMsg(sock, chatId, { text: `🗺️ *MAPA POKÉMON*\n\n📍 Você está em: *${p.cidade}*\n\n🌱 Sua jornada começa aqui. Use */viajar* para explorar novas áreas.` });
      return;
    }
    ...
    if (texto.trim().toLowerCase() === '/updates' || texto.trim().toLowerCase() === '/update') {
      await enviarMsg(sock, chatId, {
        text: `🚀 *ATUALIZAÇÕES DO COGB-AI*\n\n` +
          ...
          `🐾 *POKÉMON — BETA 0.1*\n` +
          `• /pokemon e /choose — escolha permanente do Pokémon inicial.\n` +
          `• /mapa — mostra a cidade atual do treinador.\n` +
          `• /help p — central de ajuda Pokémon.\n` +
          `• Base preparada para viagem, captura, loja, PvP e história.\n` +
          `• Arceus, trios e grandes lendários ficam reservados para futuras atualizações.\n\n` +
    ...
    if (texto.trim().toLowerCase() === '/menu') {
      await enviarMsg(sock, chatId, {
        text: `╭━━━━━━━━━━━━━━━━━━━━━━━━━━╮\n` +
          ...
          `╰━━━━━━━━━━━━━━━━━━━━━━━━━━━━╯\n\n` +
          `╭━━〔 🐾 *POKÉMON BETA 0.1* 〕━━╮\n` +
          `┃ 🐾 */pokemon* — Perfil Pokémon\n` +
          `┃ 🎒 */choose 1|2|3* — Escolher inicial\n` +
          `┃ 🗺️ */mapa* — Ver sua cidade\n` +
          `┃ 🧭 */viajar* — Explorar o mundo\n` +
          `┃ 📦 */pokemons* — Ver sua equipe\n` +
          `┃ 🏪 */shop* — Loja Pokémon\n` +
