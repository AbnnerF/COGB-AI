...
const COMANDOS_CORRIGIVEIS = [
  'menu', 'updates', 'update', 'play', 'audio', 'fig', 'forca', 'duelo', 'tops', 'kill', 'top',
  'pack', 'packs', 'convert', 'quiz', 'party', 'feedback', 'ligado',
  'rpg', 'levels', 'inimigos', 'amongus', 'among', 'task', 'report', 'vote', 'skip', 'inimigo', 'historia', 'história', 'inventario', 'inventário', 'battle', 'party', 'create', 'cancel', 'perfil', 'hug', 'punch', 'kiss', 'imagem', 'img',
];
...
const amongUsAtivos = {}; // chave: grupo -> estado da partida
const amongUsPendencias = {}; // chave: grupo:player -> escolhas privadas (task/kill/proximidade)

const AMONG_US_LUGARES = [
  'Cafeteria', 'Administração', 'MedBay', 'Elétrica', 'Segurança',
  'Armazém', 'Navegação', 'Reator', 'O2', 'Comunicações'
];

const AMONG_US_TASKS = [
  'organizar os cabos',
  'desviar energia para os sistemas',
  'baixar dados no terminal',
  'limpar o filtro de O2',
...
function nomeAmong(id) {
  return contatosCache[id] || id.split('@')[0];
}

function embaralharAmong(lista) {
  const a = [...lista];
...
function iniciarAmongUs(chatId, criadorId) {
  amongUsAtivos[chatId] = {
    active: true, fase: 'party', criadorId,
    players: [], impostores: [], vivos: [], mortos: [],
    tarefas: {}, locais: {}, totalTasks: 0, feitas: 0,
    filaEvento: null, ultimaAcao: Date.now(), meeting: null
  };
  return amongUsAtivos[chatId];
}

function encerrarAmongUs(chatId) {
  const jogo=amongUsAtivos[chatId];
  if (jogo?.filaEvento) clearTimeout(jogo.filaEvento);
  delete amongUsAtivos[chatId];
}

function progressoTasksAmong(jogo) {
  return jogo.totalTasks ? Math.round((jogo.feitas/jogo.totalTasks)*100) : 0;
}

function formatarVivosAmong(jogo) {
  return jogo.players.filter(id=>jogo.vivos.includes(id)).map(id=>`• @${id.split('@')[0]}`).join('\n');
}

async function iniciarRodadaAmong(sock, chatId) {
  const jogo=amongUsAtivos[chatId];
  if (!jogo?.active || jogo.fase!=='jogando') return;
  const vivos=jogo.vivos.filter(id=>!jogo.mortos.includes(id));
  if (jogo.impostores.every(id=>jogo.mortos.includes(id))) {
    jogo.fase='fim';
    await enviarMsg(sock,chatId,{text:'🏆 *OS IMPOSTORES FORAM DESCOBERTOS!*\n\n👨‍🚀 Os tripulantes venceram a partida!'});
    return;
  }
  if (vivos.length-jogo.impostores.filter(id=>!jogo.mortos.includes(id)).length <= jogo.impostores.filter(id=>!jogo.mortos.includes(id)).length) {
    jogo.fase='fim';
    await enviarMsg(sock,chatId,{text:'☠️ *OS IMPOSTORES DOMINARAM A NAVE!*\n\nOs impostores venceram!'});
    return;
  }

  const jogador= vivos[Math.floor(Math.random()*vivos.length)];
  const local=AMONG_US_LUGARES[Math.floor(Math.random()*AMONG_US_LUGARES.length)];
  jogo.locais[jogador]=local;

  // Às vezes cria proximidade entre dois jogadores.
  if (vivos.length>=2 && Math.random()<0.45) {
    const outros=vivos.filter(id=>id!==jogador);
    const outro=outros[Math.floor(Math.random()*outros.length)];
    jogo.locais[outro]=local;
    await enviarMsg(sock,chatId,{
      text:`👀 *MOVIMENTO NA NAVE!*\n\n@${jogador.split('@')[0]} e @${outro.split('@')[0]} estão perto de ${local}.\n\nVocês querem continuar juntos ou se afastar?`,
      mentions:[jogador,outro]
    });
  }

  if (!jogo.impostores.includes(jogador)) {
    const task=AMONG_US_TASKS[Math.floor(Math.random()*AMONG_US_TASKS.length)];
    jogo.tarefas[jogador] ??= {feitas:0,total:2};
    const pend=`${chatId}:${jogador}`;
    amongUsPendencias[pend]={tipo:'task',task,local};
    await enviarMsg(sock,jogador,{text:`🧰 *TAREFA ENCONTRADA*\n\nVocê encontrou uma tarefa em *${local}*:\n➡️ ${task}\n\nQuer fazer a tarefa?\nResponda */task sim* ou */task não*.`});
    await enviarMsg(sock,chatId,{text:`📍 @${jogador.split('@')[0]} encontrou uma tarefa em *${local}*.` ,mentions:[jogador]});
  } else {
    // O impostor recebe a oportunidade privada; nunca revelamos o papel no grupo.
    const alvo=vivos.filter(id=>id!==jogador && !jogo.impostores.includes(id))[Math.floor(Math.random()*Math.max(1,vivos.filter(id=>id!==jogador && !jogo.impostores.includes(id)).length))];
    if (alvo) {
      const pend=`${chatId}:${jogador}`;
      amongUsPendencias[pend]={tipo:'kill',alvo,local};
      await enviarMsg(sock,jogador,{text:`🕵️ *VOCÊ ESTÁ SOZINHO COM @${alvo.split('@')[0]}!*\n📍 Local: *${local}*\n\nQuer eliminar essa pessoa?\n1️⃣ */kill sim*\n2️⃣ */kill não*` ,mentions:[alvo]});
    }
  }

  // Agenda o próximo evento.
  jogo.filaEvento=setTimeout(()=>iniciarRodadaAmong(sock,chatId), 30000+Math.floor(Math.random()*30000));
}
...
async function iniciarBot() {
  ...
  sock.ev.on('messages.upsert', async ({ messages }) => {
    ...
    // -------------------- AMONG US --------------------
    if (isGroup && (comando === '/amongus' || comando === '/among') ) {
      const jogo=amongUsAtivos[chatId];
      if (jogo?.active) {
        await enviarMsg(sock,chatId,{text:'🚀 Já existe uma partida de *Among Us* neste grupo.\nUse */party amongus* para entrar na preparação.'});
        return;
      }
      iniciarAmongUs(chatId,remetenteId);
      await enviarMsg(sock,chatId,{text:'🚀 *AMONG US — SALA CRIADA!*\n\nPara escolher os participantes, use:\n*/party amongus* e mencione todos que vão jogar.\n\n⏳ A contagem de 10 segundos começa quando a party for enviada.'});
      return;
    }

    if (isGroup && comando === '/party' && /^\/party\s+among(?:us)?/i.test(textoLimpo)) {
      const jogo=amongUsAtivos[chatId];
      if (!jogo?.active || jogo.fase!=='party') {
        await enviarMsg(sock,chatId,{text:'🚀 Nenhuma sala de Among Us está aguardando participantes. Use */amongus* primeiro.'});
        return;
      }
      const ids=[...new Set([remetenteId,...mencionado])];
      if (ids.length<3) {
        await enviarMsg(sock,chatId,{text:'❌ São necessários pelo menos 3 participantes para começar uma partida.'});
        return;
      }
      jogo.players=ids;
      jogo.vivos=[...ids];
      jogo.totalTasks=ids.filter(id=>!jogo.impostores.includes(id)).length*2;
      await enviarMsg(sock,chatId,{
        text:`🚀 *PARTIDA DE AMONG US*\n\n${ids.map((id,i)=>`${i+1}. @${id.split('@')[0]}`).join('\n')}\n\n⏳ *Preparem-se! A partida começa em 10 segundos...*`,
        mentions:ids
      });
      for(let n=9;n>=1;n--) await new Promise(r=>setTimeout(r,1000));
      const qtdImpostores=ids.length>13?2:1;
      jogo.impostores=embaralharAmong(ids).slice(0,qtdImpostores);
      jogo.vivos=[...ids];
      jogo.fase='jogando';
      jogo.tarefas={};
      jogo.feitas=0;
      for(const id of ids) {
        const ehImp=jogo.impostores.includes(id);
        await enviarMsg(sock,id,{text:ehImp
          ?'🔴 *VOCÊ É O IMPOSTOR!*\n\nSeu papel é secreto. Não revele sua identidade. Observe os outros jogadores e use as opções que o bot enviar no seu PV.'
          :'🟢 *VOCÊ É UM INOCENTE!*\n\nComplete suas tarefas e tente descobrir quem são os impostores. Seu papel é secreto.'});
      }
      await enviarMsg(sock,chatId,{text:`🚀 *A PARTIDA COMEÇOU!*\n\nTodos receberam seus papéis no PV.\n🧑‍🚀 Tripulantes: ${ids.length-qtdImpostores}\n🔴 Impostores: ${qtdImpostores}\n\n📋 Completem as tarefas, observem os movimentos e usem */report* se encontrarem alguém eliminado.`});
      iniciarRodadaAmong(sock,chatId);
      return;
    }

    if (isGroup && comando === '/task') {
      const jogo=amongUsAtivos[chatId];
      const pend=amongUsPendencias[`${chatId}:${remetenteId}`];
      if (!jogo?.active || !pend || pend.tipo!=='task') return;
      const resposta=textoLower.split(/\s+/)[1];
      if (!['sim','s','não','nao','n'].includes(resposta)) {
        await enviarMsg(sock,chatId,{text:'🧰 Responda */task sim* ou */task não*.'}); return;
      }
      if (['não','nao','n'].includes(resposta)) {
        delete amongUsPendencias[`${chatId}:${remetenteId}`];
        await enviarMsg(sock,remetenteId,{text:'↩️ Você recusou a tarefa. Talvez ela apareça novamente mais tarde.'});
        return;
      }
      await enviarMsg(sock,remetenteId,{text:`🧰 *TAREFA EM ANDAMENTO*\n\nVá até *${pend.local}* e ${pend.task}.\n\nQuando terminar, responda */task concluída*.`});
      if (textoLower.includes('concluída')) {
        // mantido abaixo para facilitar a resposta em outra mensagem
      }
      if (!amongUsPendencias[`${chatId}:${remetenteId}`].aguardandoConclusao) {
        amongUsPendencias[`${chatId}:${remetenteId}`].aguardandoConclusao=true;
      }
      return;
    }

    if (!isGroup && comando === '/task') {
      // Esta forma permite concluir a tarefa diretamente pelo PV.
      for (const [chave,pend] of Object.entries(amongUsPendencias)) {
        if (chave.endsWith(`:${remetenteId}`) && pend.tipo==='task') {
          if (textoLower.includes('conclu')) {
            const [grupoId]=chave.split(':');
            const jogo=amongUsAtivos[grupoId];
            if (!jogo?.active) return;
            jogo.feitas++;
            pend.aguardandoConclusao=false;
            const feito=jogo.feitas;
            delete amongUsPendencias[chave];
            await enviarMsg(sock,remetenteId,{text:'✅ *TAREFA CONCLUÍDA!*\n\nVocê completou sua tarefa com sucesso.'});
            await enviarMsg(sock,grupoId,{text:`🧑‍🚀 @${remetenteId.split('@')[0]} terminou uma task! 📋\n\n📊 Progresso das tarefas: *${progressoTasksAmong(jogo)}%*`,mentions:[remetenteId]});
          }
          return;
        }
      }
    ...
    if (!isGroup && comando === '/kill') {
      for (const [chave,pend] of Object.entries(amongUsPendencias)) {
        if (chave.endsWith(`:${remetenteId}`) && pend.tipo==='kill') {
          const [grupoId]=chave.split(':');
          const jogo=amongUsAtivos[grupoId];
          if (!jogo?.active) return;
          const resposta=textoLower.split(/\s+/)[1];
          if (resposta==='sim' || resposta==='s') {
            if (Math.random()<0.12) {
              await enviarMsg(sock,remetenteId,{text:'⚠️ Você tentou agir, mas alguém apareceu por perto. Você decidiu recuar.'});
              return;
            }
            jogo.mortos.push(pend.alvo);
            jogo.vivos=jogo.vivos.filter(id=>id!==pend.alvo);
            await enviarMsg(sock,grupoId,{text:`☠️ *@${pend.alvo.split('@')[0]} foi encontrado eliminado em ${pend.local}.*\n\n🚨 Alguém pode ir até lá e usar */report* para iniciar uma reunião.`,mentions:[pend.alvo]});
            await enviarMsg(sock,remetenteId,{text:`🕵️ A ação aconteceu em *${pend.local}*. Ninguém no grupo recebeu seu papel.`});
            delete amongUsPendencias[chave];
          } else if (resposta==='não' || resposta==='nao' || resposta==='n') {
            await enviarMsg(sock,remetenteId,{text:'↩️ Você decidiu deixar essa pessoa em paz.'});
            delete amongUsPendencias[chave];
          } else {
            await enviarMsg(sock,remetenteId,{text:'Responda */kill sim* ou */kill não*.'});
          }
          return;
        }
      }
    ...
    if (isGroup && comando === '/report') {
      const jogo=amongUsAtivos[chatId];
      if (!jogo?.active || jogo.fase!=='jogando') return;
      const mortos=jogo.mortos;
      if (!mortos.length) {
        await enviarMsg(sock,chatId,{text:'📢 Não há ninguém eliminado para reportar agora.'}); return;
      }
      jogo.fase='reuniao';
      jogo.meeting={votes:{}, reporter:remetenteId};
      await enviarMsg(sock,chatId,{text:`🚨 *REUNIÃO DE EMERGÊNCIA!*\n\n@${remetenteId.split('@')[0]} reportou um corpo.\n\n🗳️ Para votar: */vote @pessoa*\n⏭️ Para pular: */skip*\n\nA pessoa com mais votos será ejetada.`,mentions:[remetenteId]});
      return;
    }

    if (isGroup && (comando === '/vote' || comando === '/skip')) {
      const jogo=amongUsAtivos[chatId];
      if (!jogo?.active || jogo.fase!=='reuniao') return;
      if (!jogo.vivos.includes(remetenteId)) {
        await enviarMsg(sock,chatId,{text:'👻 Jogadores eliminados não podem votar.'}); return;
      }
      const alvo=comando==='/skip' ? 'skip' : mencionado[0];
      if (!alvo) { await enviarMsg(sock,chatId,{text:'🗳️ Use */vote @pessoa* ou */skip*.'}); return; }
      jogo.meeting.votes[remetenteId]=alvo;
      const totalVotos=Object.keys(jogo.meeting.votes).length;
      await enviarMsg(sock,chatId,{text:`🗳️ Voto registrado de @${remetenteId.split('@')[0]}.\n\n${totalVotos}/${jogo.vivos.length} votos registrados.`,mentions:[remetenteId]});
      if (totalVotos>=jogo.vivos.length) {
        const cont={};
        for(const v of Object.values(jogo.meeting.votes)) cont[v]=(cont[v]||0)+1;
        const ordenado=Object.entries(cont).sort((a,b)=>b[1]-a[1]);
        const top=ordenado[0];
        if(!top || (ordenado[1] && ordenado[1][1]===top[1]) || top[0]==='skip') {
          await enviarMsg(sock,chatId,{text:'⏭️ *Votação encerrada.*\nNinguém foi ejetado.'});
        } else {
          const expulso=top[0];
          jogo.mortos.push(expulso);
          jogo.vivos=jogo.vivos.filter(id=>id!==expulso);
          const eraImp=jogo.impostores.includes(expulso);
          await enviarMsg(sock,chatId,{text:`🚀 @${expulso.split('@')[0]} foi ejetado da nave.\n\n${eraImp?'🔴 Era um impostor!':'🟢 Era um inocente!'}`,mentions:[expulso]});
        }
        jogo.fase='jogando';
        jogo.meeting=null;
        setTimeout(()=>iniciarRodadaAmong(sock,chatId),5000);
      }
      return;
    }
    ...
    if (texto.trim().toLowerCase() === '/updates' || texto.trim().toLowerCase() === '/update') {
      await enviarMsg(sock, chatId, {
        text: `🚀 *ATUALIZAÇÕES DO COGB-AI*\n\n` +
          ...
          `💀 *Novo comando*\n` +
          `• /kill @pessoa — elimina o jogador apenas como brincadeira, no estilo Minecraft\n\n` +
          `💋 *Nova ação*\n` +
          `• /kiss @pessoa — envia um GIF de beijo com os participantes mencionados\n\n` +
          `🎮 *Brincadeiras*\n` +
          `• /forca — jogo da Forca\n` +
