...
// chave -> { fase: 'quantidade'|'fotos', total, figurinhas, timeoutHandle }
...
async function iniciarBot() {
  ...
  sock.ev.on('messages.upsert', async ({ messages }) => {
    ...
    if (msg.message.imageMessage && aguardandoPack[chaveEspera]?.fase === 'fotos') {
      ...
      try {
        ...
        const figurinha = await criarFigurinhaDeImagem(bufferImagem, legenda, 'recortada');
        if (!figurinha) {
          sessaoPack.timeoutHandle = setTimeout(() => {
            delete aguardandoPack[chaveEspera];
          }, 10 * 60 * 1000);
          await enviarMsg(sock, chatId, { text: '❌ Não consegui transformar essa foto em figurinha. Manda outra foto.' });
          return;
        }

        sessaoPack.figurinhas.push(figurinha);
        ...
        if (recebidas >= sessaoPack.total) {
          ...
          await enviarMsg(sock, chatId, {
            text: `🎉 *Pack criado com sucesso!*\n\n📦 Figurinhas: *${recebidas}/${sessaoPack.total}*\n💡 As legendas das fotos foram usadas como nome de cada figurinha quando informadas.`,
          });

          for (const figurinhaPronta of sessaoPack.figurinhas) {
            await enviarMsg(sock, chatId, { sticker: figurinhaPronta });
          }
          return;
        }
        ...
        await enviarMsg(sock, chatId, {
          text: `✅ ${recebidas}/${sessaoPack.total} figurinha(s) recebida(s)!\n\nManda a próxima foto. 📸\n\nPara cancelar: */cancel pack*`,
        });
        return;
      } catch (err) {
        console.log('Erro ao criar figurinha do pack:', err.message);
        sessaoPack.timeoutHandle = setTimeout(() => {
          delete aguardandoPack[chaveEspera];
        }, 10 * 60 * 1000);
    ...
    if (aguardandoPack[chaveEspera]?.fase === 'quantidade') {
      ...
      await enviarMsg(sock, chatId, {
        text: `✅ Beleza! Vou criar um pack com *${quantidade} figurinha(s)*.\n\nAgora mande as *${quantidade} fotos*, uma por vez. 📸\n\n💡 Quer dar um nome para uma figurinha? Coloque o nome na *legenda da foto*.\n\n📊 Progresso: *0/${quantidade}*\n🛑 Para cancelar: */cancel pack*`,
      });
      return;
    }
    ...
    // Comando /pack <tema> - busca packs públicos do tema e envia figurinhas aleatórias
    if (textoLower.startsWith('/pack ')) {
      const temaPack = textoLimpo.slice(6).trim();
      if (!temaPack) {
        await enviarMsg(sock, chatId, {
          text: '📦 Use assim: /pack memes\nExemplo: /pack Dragon Ball\n\nVou procurar packs públicos do tema e escolher figurinhas aleatórias.',
        });
        return;
      }
      ...
      await enviarMsg(sock, chatId, {
        text: `🔎 *Procurando figurinhas de: ${temaPack}*\n\nVou procurar packs públicos e escolher figurinhas aleatórias. ⏳`,
      });

      const resultadoPack = await baixarPackStickerLy(temaPack, 6);
      const pack = resultadoPack.figurinhas || [];

      if (!pack.length) {
        await enviarMsg(sock, chatId, {
          text: `❌ Não consegui encontrar figurinhas utilizáveis para *${temaPack}* agora.\n\nTenta outro nome ou tema.`,
        });
        return;
      }

      await enviarMsg(sock, chatId, {
        text: `✅ *Pack encontrado!*\n\n🎯 Tema: *${temaPack}*\n🎲 Figurinhas aleatórias: *${pack.length}/6*\n\nCada pedido tenta escolher figurinhas diferentes das usadas recentemente.`,
      });

      for (const figurinha of pack) {
        await enviarMsg(sock, chatId, { sticker: figurinha });
      }
      return;
    }
