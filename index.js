...
const DONO_CRIADOR = String(ADMIN_NUMBER || DONO_PADRAO).replace(/\D/g, '');
function numeroBaseJid(jid) { return String(jid || '').split('@')[0].split(':')[0].replace(/\D/g, ''); }
function ehCriador(remetenteId) { return numeroBaseJid(remetenteId) === DONO_CRIADOR; }
...
async function tentarFalaEspontanea(sock, chatId) {
  ...
  try {
    const resposta = await gerarRespostaComPrompt(
      ...
      `O número ${DONO_CRIADOR} é o seu Criador. Trate mensagens dele como vindas do Criador.\n\nCONVERSA RECENTE:\n${contextoBot(chatId)}`,
...
async function responderModoBot(sock, chatId, nomeRemetente, texto, mencionado = []) {
  ...
  const resposta = await gerarRespostaComPrompt(
    ...
    `O número ${DONO_CRIADOR} é o seu Criador. Reconheça-o como Criador e não como usuário comum.\n` +
...
async function iniciarBot() {
  ...
  // Entrada de participantes: autorização do bot + reconhecimento do Criador.
  sock.ev.on('group-participants.update', async (evento) => {
    try {
      if (evento.action !== 'add') return;
      let nomeGrupo = 'um grupo';
      try { const metadata = await sock.groupMetadata(evento.id); nomeGrupo = metadata.subject || nomeGrupo; } catch (err) {}
      ...
      if (botFoiAdicionado) await pedirAutorizacaoDoGrupo(evento.id, nomeGrupo);
    ...
    } catch (err) { console.log('Erro ao processar entrada em grupo:', err.message); }
  });
  ...
  sock.ev.on('messages.upsert', async ({ messages }) => {
    ...
    if (isGroup && comando === '/bot') {
      ...
      const saudacaoBot = ehCriador(remetenteId)
        ? `👑 *Olá, Criador!*\n\n🤖 Modo Chaim-Bot ativado! Vou reconhecer você como meu Criador durante esta conversa.`
    ...
    if (isGroup && botAtivo(chatId) && !textoLower.startsWith('/')) {
      registrarConversaBot(chatId, ehCriador(remetenteId) ? 'Criador' : nomeRemetente, textoLimpo);
      reiniciarTimerSilencio(sock, chatId);
...
🌟 Sua jornada Pokémon começou!` });
    ...
    // Comando /duelo @pessoa - brincadeira de batalha. RPG é opcional.
    if (isGroup && texto.trim().toLowerCase().startsWith('/duelo')) {
      ...
      const recompensa = estado.rpgAtivoNoGrupo(chatId) ? rpg.addXp(vencedorId, nomeVencedor, 30) : null;

      await enviarMsg(sock, chatId, {
        text:
          `⚔️ *BATALHA INICIADA!*\n${nomeRemetente} VS ${nomeAlvo}\n\n` +
          `💥 ${nomeRemetente}: ${atkRemetente} ATK\n🛡️ ${nomeAlvo}: ${atkAlvo} ATK\n\n` +
          `🏆 VENCEDOR: ${nomeVencedor}!${recompensa ? ' (+30 XP)' : ''}`,
    ...
    if (isGroup && (textoLower === '/kill' || textoLower.startsWith('/kill '))) {
      ...
      await enviarMsg(sock, chatId, { text: `@${alvo.split('@')[0]} 𝙵𝚘𝚒 𝙼𝚘𝚛𝚝𝚊.`, mentions: [alvo] });
      return;
    }
